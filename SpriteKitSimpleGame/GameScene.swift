...
class GameScene: SKScene, SKPhysicsContactDelegate
{
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?)
    {
        ...
        // Ask the player to fire a bullet
        if let bullet = player.shootBullet(touchLocation: touchLocation)
        {
            self.addChild(bullet)
        }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?)
    {
        ...
        let left = CGPoint (x: self.size.width * 0.1, y: self.size.height * 0.1)
        let right = CGPoint (x: self.size.width * 0.9, y: self.size.height * 0.1)
        
        
        if (left.dist(pt: touchLocation) <= 50.0)
        {
            player.moveDir = Direction.LEFT;
        }
        else if (right.dist(pt: touchLocation) <= 50.0)
        {
